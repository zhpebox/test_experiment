...
public class BDFclass {
	...
	/**
	 * 队列对头节点出对列，孩子节点入队列，如当前节点是层次尾节点，深度+1，并在队尾设置新的层次结束标识。
	 * author ZHP
	 * 2014年11月26日
	 * @param seqQueue
	 * @param outDegreeList
	 ...
	 * @return
	 */
	public ArrayList<BDFResultNode> IterateTheArrayListExcludeCore(ArrayList<AdjArrayObject>[] outDegreeList,ArrayList<node> nodeList) {
		...
		for (node nodeListObj : nodeList) {
			...
			while (!seqQueue.isEmpty()) {
				// 对头出队列，孩子入队列
				int temp = this.BDF_DeQueueExcludeCore(seqQueue, outDegreeList);
				System.out.print(" " + temp);
				nodeResult = nodeResult+temp+",";
			}
			System.out.println("\nNode "+currentNode+": breadth is " + breadth + " ; depth is " + depth);
	...
	public Integer BDF_DeQueueExcludeCore(Queue seqQueue,ArrayList<AdjArrayObject>[] outDegreeList){
		//lineMark判断当前的是否行结束标志
		int lineMark = 0;
		int temp = seqQueue.DeQueue();
		if(temp<0) {
		...
		if(currentLine!=null && currentLine.size()!=0){
			for (int i = 0; i < currentLine.size(); i++) {
				...
				CurrentNode.setIndex(currentLine.get(i).getNodeIndex());
				
				if(seqQueue.isExistTheSameOne(currentLine.get(i).getNodeIndex())  ||  cnodes.contains(CurrentNode)){
					continue;
				}else{
					seqQueue.EnQueue(currentLine.get(i).getNodeIndex());
				}
	...
	public ArrayList<node> getCnodes() {
		return cnodes;
	}

	public void setCnodes(ArrayList<node> cnodes) {
		this.cnodes = cnodes;
	}
